...
It makes more sense to wrap them around because if you just stacked them all in one column they would get smushed and same with if we only had one row. You would barely, or not at all, be able to see the data. This layout made the graphs more square like which allows me to see the most data.
...
top_neighbourhoods <- edibnb %>%
  group_by(neighbourhood) %>%                         
  summarize(median_price = median(price, na.rm=TRUE)) %>% 
  arrange(desc(median_price)) %>%                    
  head(5) 
  
top_neighbourhood_vector <- top_neighbourhoods %>%
  pull(neighbourhood)
...
filtered_data <- edibnb %>%
  filter(neighbourhood == top_neighbourhood_vector)

ggplot(filtered_data, aes(x = price, y = neighbourhood, fill = neighbourhood)) +
  geom_density_ridges() +
  labs(title = "Distribution of Listing Prices in Top 5 Neighbourhoods", 
       x = "Listing Price", 
       y = "Neighbourhood")
```
...
Most people rate these places fairly high with a few outliers. The location doesn't matter too much because every place has majority of the data above a 90.
